...
public class CustEnquiryController {
	...
	public String customerFolloupPage(
			Model model,
			@RequestParam(name = "selectedDate" ,required = false)String selectDate) 
	{
		if (selectDate!=null) 
		{
			List<CustFollowup> list_followups = custFollowupService.getFollowupForProvidedDate(selectDate);
	...
	@ResponseBody
	public String checkPhoneNo(@RequestParam("phoneNumber")String phoneno,Model model)
	{
	...
	{
		List<CustEnquiry> list_custeqn = custEnquiryService.getCustAllEnquiryHistory(phoneno);
		model.addAttribute("model_custenq", list_custeqn);
		model.addAttribute("modelCustEnquiryAttr", new CustEnquiryModel());
