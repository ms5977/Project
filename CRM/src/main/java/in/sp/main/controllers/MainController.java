...
public class MainController {
	...
	{
		List<Employee> lis_emp = empService.getAllEmployeeService();
		
		int TOTAL_PRODUCTS=lis_emp.size();
		...
		List<Employee> new_list_emp = lis_emp.subList(LIST_START_INDEX, LIST_END_INDEX);
	...
	
	{
		...
		else
		{
			...
			{
				model.addAttribute("model_error", "Email and didn't matched");
				page="login";
			}
									  ...
									  ) 
	{
		if (!br.hasErrors()) 
		{
			...
			{
				model.addAttribute("model_error", "Employee not added successfully");
			}
			emp.setName("");
			emp.setEmail("");
			emp.setPassword("");
...
//		System.out.println("Email"+email);
		...
		if (status) {
			redirectAttributes.addAttribute("model_success", "Employee Deleted Successfully");
		}
		else {
			redirectAttributes.addAttribute("model_error", "Employee not deleted due to some error");
